...
var jxl = jxl || (function () {
    ...
    lib.find = function (selector, parentIdOrElement) {
        var parent = lib.get(parentIdOrElement) || doc;
        var result;
    ...
    lib.addClass = function (idOrElement, strClasses) {
        var elem = lib.get(idOrElement);
        if (elem) {
            var theClasses = strClasses.split(/\s+/g);
            var re;
    ...
    lib.addEventHandler = function (idOrElement, eventName, handlerFunction) {
        ...
        if (elem) {
            if (elem.addEventListener) {
                ...
                if (oldPage && oldPage.registerEventHandler) {
                    oldPage.registerEventHandler(idOrElement, eventName, handlerFunction);
                }
                return true;
            }
    ...
    lib.submitForm = function (name) {
        var frm = doc.forms[name || 0];
        if (frm) {
            if (oldPage && oldPage.takeoverSubmit(frm)) {
                return false;
            } else {
                frm.submit();
            }
    ...
    lib.setHiddenValue = function (name, value, formIdOrElement) {
        var form = lib.get(formIdOrElement) || doc.forms[0];
        if (form) {
    ...
    lib.removeElements = function (idsOrElements) {
        var i = (idsOrElements || []).length || 0;
        while (i--) {
            var elem = lib.get(idsOrElements[i]);
    ...
    lib.mergeObjects = function (defaultObj, updateObj) {
        defaultObj = defaultObj || {};
        updateObj = updateObj || {};
        var result = {};
        for (var name in defaultObj) {
            if (defaultObj.hasOwnProperty(name)) {
                result[name] = defaultObj[name];
            }
        }
        for (var name in updateObj) {
    ...
    lib.setTimeout = function (fn, delay) {
        if (oldPage) {
            return oldPage.setAndRegisterTimeout(fn, delay);
        } else {
            return setTimeout(fn, delay);
        }
    };
    lib.goTo = function (url) {
        if (oldPage) {
            oldPage.submitLink(url);
        } else {
            location.href = url;
        }
    };
